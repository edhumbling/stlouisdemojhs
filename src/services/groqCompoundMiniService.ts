/**
 * Groq Compound Mini Service
 * Uses Groq Compound Mini model with web search tools for internet search
 */
...
class GroqCompoundMiniService {
  ...
  constructor() {
    ...
    console.log('🌐 Groq Compound Mini Service initialized for web search');
    console.log('🔑 API Key:', this.apiKey ? `${this.apiKey.substring(0, 20)}...` : 'Not set');
    console.log('🌐 Endpoint:', this.apiEndpoint);
  }

  /**
   * Generate a response using Groq Compound Mini with web search tools
   */
  async generateResponse(
    ...
    console.log('🚀 Groq Compound Mini Request:', {
    ...
    if (!response.ok) {
      ...
      console.error('❌ Groq Compound Mini Error:', response.status, errorData);
      ...
      if (response.status === 429) {
        ...
        throw new Error(`Groq Compound Mini error: ${response.status} - ${JSON.stringify(errorData)}`);
      }
    }
    ...
    if (!data.choices || data.choices.length === 0) {
      throw new Error('No response generated from Groq Compound Mini');
    }

    const choice = data.choices[0];
    if (!choice.message.content) {
      throw new Error('Empty response from Groq Compound Mini');
    }

    // Log usage statistics if available
    if (data.usage) {
      console.log('📊 Groq Compound Mini Usage:', {
...
Remember: You are representing St. Louis Demonstration JHS, so always be professional, helpful, and encouraging. Your goal is to support students, parents, and the school community with accurate and useful information, enhanced by real-time web search capabilities.`;
  ...
  public async testConnection(): Promise<boolean> {
    try {
      ...
      console.log('✅ Groq Compound Mini test successful:', testResponse.substring(0, 50) + '...');
      return true;
    } catch (error) {
...
const groqCompoundMiniService = new GroqCompoundMiniService();
export default groqCompoundMiniService;